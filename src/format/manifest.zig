...
/// Segment IDs must be sorted, with the active segment being last
pub const Manifest = struct {
    ...
    pub fn encode(self: Manifest, output: []u8) Error![]u8 {
        ...
        const header_checksum = Crc32c.hash(bytes[0..44]);
        std.mem.writeInt(u32, bytes[44..48], header_checksum, .little);
        ...
        const checksum_offset = len - 4;
        const checksum = Crc32c.hash(bytes[0..checksum_offset]);

        std.mem.writeInt(u32, bytes[checksum_offset..][0..4], checksum, .little);
...
pub fn decode(bytes: []const u8, segment_ids: []u64) Error!Manifest {
    ...
    const expected_header_checksum = std.mem.readInt(u32, bytes[44..48], .little);
    const actual_header_checksum = Crc32c.hash(bytes[0..44]);

    if (expected_header_checksum != actual_header_checksum) return error.ChecksumMismatch;

    const flags = std.mem.readInt(u16, bytes[6..8], .little);
    const reserved = std.mem.readInt(u32, bytes[40..44], .little);
    ...
    const checksum_offset = len - 4;
    const expected_checksum = std.mem.readInt(u32, bytes[checksum_offset..][0..4], .little);
    ...
    const active_segment = std.mem.readInt(u64, bytes[32..40], .little);
    if (active_segment != previous) return error.InvalidActiveSegment;
    ...
    for (segment_ids[0..count], 0..) |*id, index| {
        id.* = readId(bytes, index);
    }
