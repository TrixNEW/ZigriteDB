...
pub fn compactBatch(index: *const index_module.Index, segment_id: u64, batch: Batch, output: []u8) ![]const u8 {
    ...
    while (read < batch.records.len) {
        ...
        // The final batch preserves the last batch ID, even if no keys remain
