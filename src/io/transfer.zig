...
/// Errors may leave the buffer partly filled.
pub fn readExact(device: anytype, buffer: []u8, offset: u64) !void {
    try checkRange(offset, buffer.len);
    var done: usize = 0;
    while (done < buffer.len) {
        const count = try device.readSome(buffer[done..], offset + done);
...
pub fn writeAll(device: anytype, bytes: []const u8, offset: u64) !void {
    ...
    while (done < bytes.len) {
        const count = try device.writeSome(bytes[done..], offset + done);
...
fn checkRange(offset: u64, len: usize) error{InvalidOffset}!void {
    _ = std.math.add(u64, offset, len) catch return error.InvalidOffset;
}
